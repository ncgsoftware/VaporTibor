...
final class UtilityDatabaseTransferCommand: Command {
    ...
    func run(using context: CommandContext, signature: Signature) throws {
        ...
        let frames = ["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"].map { $0 + " Transfer in progress" }
        
        let loadingBar = context.console.customActivity(frames: frames)
        
        let sourceDb = app.databases.database(.sqlite, logger: app.logger, on: app.eventLoopGroup.next())!
        
        let targetDb = app.databases.database(.psql, logger: app.logger, on: app.eventLoopGroup.next())!
        ...
        do {
            ...
            try BlogCategoryModel.query(on: sourceDb).all()
                .flatMap {
                    $0.map { BlogCategoryModel(id: $0.id, title: $0.title) }
                    .create(on: targetDb)
            }.wait()
            
            try BlogPostModel.query(on: sourceDb).with(\.$category).all()
                .flatMap {
                    $0.map {
                        BlogPostModel(id: $0.id, title: $0.title, slug: $0.slug, image: $0.image, imageKey: $0.imageKey, excerpt: $0.excerpt, date: $0.date, content: $0.content, categoryId: $0.category.id!)
                        }.create(on: targetDb)
                }.wait()
            loadingBar.succeed()
        } catch {
