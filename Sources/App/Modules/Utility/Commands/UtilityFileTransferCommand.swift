...
final class UtilityFileTransferCommand: Command {
    ...
    func run(using context: CommandContext, signature: Signature) throws {
        ...
        var publicPath = app.directory.publicDirectory
        if publicPath.last! == "/" { publicPath.removeLast() }
        ...
        do {
            ...
            let originalModels = models.filter { $0.imageKey == nil && $0.image.hasPrefix("/images/posts/") }
            
            for model in originalModels {
                
                let key = "/blog/posts/" + UUID().uuidString + ".jpg"
                
                try FileManager.default.moveItem(atPath: publicPath + model.image, toPath: assetsPath + key)
