...
extension AdminViewController {
    ...
    func render(req:Request, form: EditForm) -> EventLoopFuture<View> {
        self.beforeRender(req: req, form: form)
    ...
    func update(req: Request) throws -> EventLoopFuture<View> {
        ...
        return form.validate(req: req)
            .flatMap { isValid in
                guard isValid else { return self.render(req: req, form: form) }
                
                do {
                    return try self.find(req)
                        .flatMap { model in self.beforeUpdate(req: req, model: model, form: form) }
                        .flatMap { model in
                            form.write(to: model as! EditForm.Model)
                        ...
                        .flatMap { self.render(req: req, form: form) }
                } catch { return req.eventLoop.future(error: error) }
            }
    }
